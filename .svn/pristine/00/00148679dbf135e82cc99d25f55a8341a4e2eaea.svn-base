...
class BasicAuthentication extends BaseController
{
    ...
    public function IsAuthenticated($headervalues)
    {
        ...
        else
        {
            ...
            if($api_username != "" && $api_password != "")
            {
                ...
                $customer = new CustomerRepository($customerObj);
                $isMatch = $customer->getCustomerCountBy($api_username, $api_password);
                if($isMatch)
                {
                    return 0;
